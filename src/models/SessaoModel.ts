...
export class SessaoModel implements SessaoModelProps {
  ...
  static async read() {
    ...
    console.table(sessoesDetalhadas, [
      "nome_sala",
      "nome_filme",
      "horario_inicio",
    ]);
  ...
  static async find(sessao_id: ObjectId): Promise<SessaoModel | null> {
    const collection = await this.getCollection();
    const sessao = await collection.findOne<SessaoModel>({ _id: sessao_id });
    return sessao ? new SessaoModel(sessao as SessaoModelProps) : null;
  }

  static async findByFilme(filme_id: ObjectId): Promise<SessaoModel[]> {
    const collection = await this.getCollection();
    const sessoes = await collection.find<SessaoModel>({ filme_id }).toArray();
