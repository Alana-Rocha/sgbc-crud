...
export default async function populateDatabase() {
  ...
  if (!(await collectionExists(db, 'clientes'))) {
    ...
    await db.collection('clientes').insertMany(clientes, { ordered: false });
    populated = true;
  }
  ...
  if (!(await collectionExists(db, 'filmes'))) {
    const filmes = [
      ...
      { titulo: 'Candelabro, a história não contada', duracao: 150, genero: 'Drama' },
  ...
  if (!(await collectionExists(db, 'sessoes'))) {
    ...
    const salasInseridas = await db.collection('salas').find().toArray();
    const sessoes = [
      { filme_id: filmesInseridos[0]._id, sala_id: salasInseridas[0]._id, horario_inicio: '2024-10-16 10:00:00' },
      { filme_id: filmesInseridos[1]._id, sala_id: salasInseridas[1]._id, horario_inicio: '2024-10-16 13:00:00' },
      { filme_id: filmesInseridos[2]._id, sala_id: salasInseridas[2]._id, horario_inicio: '2024-10-16 16:00:00' },
      { filme_id: filmesInseridos[3]._id, sala_id: salasInseridas[3]._id, horario_inicio: '2024-10-16 19:00:00' },
      { filme_id: filmesInseridos[4]._id, sala_id: salasInseridas[4]._id, horario_inicio: '2024-10-16 21:00:00' },
    ];
  ...
  if(populated) {
    const collectionsPopulated = ['clientes', 'filmes', 'salas', 'poltronas', 'sessoes', 'ingressos'];
    const missingCollections = [];

    for (let collection of collectionsPopulated) {
